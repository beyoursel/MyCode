...
def greedy_matching(cost_matrix):
    ...
    index_1d = np.argsort(distance_1d) # from small to big 
...
def data_association(dets, trks, metric, threshold, algm='greedy', \
    ...
    Returns 3 lists of matches, unmatched_dets and unmatched_trks, and affinity matrix
    """

    # if there is no item in either row/col, skip the association and return all unmatched
    ...
    if hypothesis == 1:
        if algm == 'hungar':
            ...
            print("please choose right algorithm for association!")
...
if __name__ == '__main__':
    cost_matrix = np.random.randn(6, 8)
