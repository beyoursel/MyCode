...
def polygon_clip(subjectPolygon, clipPolygon):
	...
	Args:
		...
		clipPolygon: a list of (x,y) 2d points, has to be *convex* 凸的
	Note:
		**points have to be counter-clockwise ordered** 顺时针连续的点

	Return:
		a list of (x,y) vertex point for the intersection polygon. 返回交集多边形的顶点
	"""
	def inside(p):
	...
	cp1 = clipPolygon[-1] # 保存多边形尾部的顶点坐标
 
	for clipVertex in clipPolygon: # 遍历其中一个多边形的顶点
		cp2 = clipVertex 
		inputList = outputList # 这里的inputList保存了另外一个多边形的顶点
		outputList = []
		s = inputList[-1] # 保存多边形尾部的顶点坐标
 
		for subjectVertex in inputList:
			e = subjectVertex # 保存头顶点坐标
			if inside(e):
...
def compute_height(box_a, box_b, inter=True):
	...
	corners1 = Box3D.box2corners3d_lidar(box_a) 	# 8 x 3
	corners2 = Box3D.box2corners3d_lidar(box_b)	# 8 x 3
	...
	if inter: 		# compute overlap height
		...
		ymax = max(corners1[0, 2], corners2[0, 2])
		ymin = min(corners1[4, 2], corners2[4, 2])
		height = max(0.0, ymax - ymin)
		# if height == 0:
		# 	print("debug\n")
...
def compute_height_cam(box_a, box_b, inter=True):
...
def compute_bottom(box_a, box_b):
	...
	# boxa_bot = corners1[-5::-1, [0, 2]] 		# 4 x 2 获得底部的corners 这里的[-5::-1]表示将顺序反转
	# boxb_bot = corners2[-5::-1, [0, 2]]			# 4 x 2

	boxa_bot = corners1[-4:, [0, 1]] 		# 4 x 2 获得底部的corners 这里的[-5::-1]表示将顺序反转
	boxb_bot = corners2[-4:, [0, 1]]		
