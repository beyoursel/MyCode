...
class VoxelResBackBone8xVoxelNext_VV(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        out = self.conv_out(out).dense() # spatial_features
...
class VoxelResBackBone8xVoxelNext_V3(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
class VoxelResBackBone8xVoxelNext_V4(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.shared_conv = spconv.SparseSequential(
            ...
            nn.BatchNorm1d(out_channel),
            nn.ReLU(True),
        ) # out_channel changed from 128 to 64
    ...
    def forward(self, batch_dict):
        ...
        input_sp_tensor = spconv.SparseConvTensor(
            ...
            spatial_shape=self.sparse_shape,
            batch_size=batch_size
        ) # 将体素编码后的特征用spconv.SparseConvTensor封装
        x = self.conv_input(input_sp_tensor)

        x_conv1 = self.conv1(x)
        x_conv2 = self.conv2(x_conv1) # down-sampling=2, [21, 752, 752]
        x_conv3 = self.conv3(x_conv2) # down-sampling=4, [11, 376, 376]
        x_conv4 = self.conv4(x_conv3) # down-sampling=8, [6, 188, 188], paddding[1, 1, 1]
        x_conv5 = self.conv5(x_conv4) # down-sampling=16, [3, 94, 94], padding[1, 1, 1]
        x_conv6 = self.conv6(x_conv5) # down-sampling=32, [2, 47, 47], padding[1, 1, 1]
