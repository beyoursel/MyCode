...
class SEDLayer(spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        x = features[-1]
        for deconv, norm, up_x in zip(self.decoder, self.decoder_norm, features[:-1][::-1]):  # encoder-decoder structure
            x = deconv(x)
...
class HEDNet_V3(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.conv3 = spconv.SparseSequential(
            SEDLayer(64, down_kernel_size, down_stride, num_SBB, norm_fn=norm_fn, indice_key='sedlayer3'),
            post_act_block(64, 128, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv3', conv_type='spconv'),
        )

        self.conv4 = spconv.SparseSequential(
            SparseBasicBlock(128, 128, bias=False, norm_fn=norm_fn, indice_key='res4'),
            SparseBasicBlock(128, 128, bias=False, norm_fn=norm_fn, indice_key='res4'),
        )

        last_pad = 0
        last_pad = model_cfg.get('last_pad', last_pad)
