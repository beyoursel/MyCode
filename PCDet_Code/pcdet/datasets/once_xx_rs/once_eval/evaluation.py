...
def get_evaluation_results(gt_annos, pred_annos, classes,
    ...
    if difficulty_mode == 'Distance':
        ...
        num_difficulties = 4
        difficulty_types = ['overall', '0-30m', '30-50m', '50-100m']
    else:
...
def filter_data(gt_anno, pred_anno, difficulty_mode, difficulty_level, class_name, use_superclass):
    ...
    if difficulty_mode == 'Overall':
        ignore = overall_filter(gt_anno['gt_boxes'])
        gt_flag[ignore] = 1
        ignore = overall_filter(pred_anno['pred_lidar'])
        pred_flag[ignore] = 1
    elif difficulty_mode == 'Distance':
        ignore = distance_filter(gt_anno['gt_boxes'], difficulty_level)
        gt_flag[ignore] = 1
        ignore = distance_filter(pred_anno['pred_lidar'], difficulty_level)
        pred_flag[ignore] = 1
    elif difficulty_mode == 'Overall&Distance':
        ignore = overall_distance_filter(gt_anno['gt_boxes'], difficulty_level)
        gt_flag[ignore] = 1
        ignore = overall_distance_filter(pred_anno['pred_lidar'], difficulty_level)
        pred_flag[ignore] = 1
    else:
        raise NotImplementedError
...
def compute_iou3d(gt_annos, pred_annos, split_parts, with_heading):
    ...
    for num_part_samples in split_parts:
        ...
        gt_boxes = np.concatenate([anno["gt_boxes"] for anno in gt_annos_part], 0)
        pred_boxes = np.concatenate([anno['pred_lidar'] for anno in pred_annos_part], 0)
