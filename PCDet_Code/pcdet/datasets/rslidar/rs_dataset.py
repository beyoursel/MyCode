...
class RobosenseDataset(DatasetTemplate):
    ...
    def __init__(self,dataset_cfg,class_names,training= True, root_path=None,logger = None):
        ...
        self.train_ratio_of_all_labels = 0.8 # 数据集划分比例
    ...
    def get_files_name_list(self):
        folder_list = []
        folder_list = self.get_folder_list(self.root_path)  # 获得数据集下文件夹名组成的列表

        files_list_pcd = []  # 将文件夹中所有的pcd文件整合到该文件夹下
        files_list_label = [] # 将文件夹中所有label文件整合到该文件夹下

        for per_folder in folder_list:  # 遍历每个文件夹

            #一条路的文件夹的路径one_road_path
            one_road_path = str(self.root_path+per_folder+'/')
            #一条路下文件夹下的文件列表 one_road_list =['label','pcd']
            one_road_list = self.get_folder_list(one_road_path)  # 读取子数据集文件夹下的文件名

            for one_folder in one_road_list:
                if one_folder == 'points':
    ...
    def from_label_path_to_pcd_path(self,single_label_path):
        ...
        if strl1 in single_label_path:
            single_pcd_path = single_label_path.replace(strl1,'points')
        if strl2 in single_pcd_path:
            single_pcd_path = single_pcd_path.replace(strl2,'.npy')
        #由此得到了label对应的pcd文件的路径 ：single_pcd_path
        return single_pcd_path

    
    # 根据label文件路径列表，返回所有标签的数据
    def get_all_labels(self,num_workers = 1,files_list_label=None):
        ...
        def get_single_label_info(single_label_path):
            ...
            #打开文件
            with open(single_label_path,encoding = 'utf-8') as f:  # 编码方式为utf-8
                labels = json.load(f)
            ...
            maskmotorcycle=single_objects_label_info['name']=='motorcycle'  # 若为motorcycle则为ture
            ind=np.nonzero(maskmotorcycle)  # 返回非零indices，即为motorcycle
            single_objects_label_info['name'][ind]='cyclist'  # 将motorcycle的标签改为cyclist
            maskbicycle = single_objects_label_info['name'] == 'bicycle'
            ind=np.nonzero(maskbicycle)
            single_objects_label_info['name'][ind]='cyclist'  # 将bicycle的标签改为cyclist
            masktricycle=single_objects_label_info['name']=='tricycle'
            ind=np.nonzero(masktricycle)
            single_objects_label_info['name'][ind]='cyclist'  # 将tricycle的标签改为cyclist
            maskhuge_vehicle=single_objects_label_info['name']=='huge_vehicle'
            ind=np.nonzero(maskhuge_vehicle)
            single_objects_label_info['name'][ind]='vehicle2'  # 将huge_vehicle的标签改为vehicle2
            maskbig_vehicle = single_objects_label_info['name'] == 'big_vehicle'
            ind=np.nonzero(maskbig_vehicle)
            single_objects_label_info['name'][ind]='vehicle2'  # 将big_vehicle的标签改成vehicle2


            single_objects_label_info['box_center'] = np.array([[label['center']['x'], label['center']['y'],label['center']['z']]  for  label in labels['labels']])
            single_objects_label_info['box_size'] = np.array([[label['size']['x'],label['size']['y'],label['size']['z']] for label in labels['labels']])
    ...
    def get_single_pcd_info(self,single_pcd_path):
        # print(single_pcd_path)
        single_pcd_path = single_pcd_path
        #ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        # pathx = str(ROOT_DIR)
        #pathx = '/home/wanghai/Envs-perc-team/taole'
        #single_pcd_path = pathx + '/' + single_pcd_path
        path_fixx = (Path(__file__).resolve().parent / '../../../').resolve()
        # print(path_fixx)        
        single_pcd_path  = str(path_fixx) + '/' + '/'.join(single_pcd_path.split('/')[-6:])
        single_pcd_points_np = np.load(single_pcd_path)
    ...
    def create_groundtruth_database(self,info_path = None,used_classes =None,split = 'train'):
        ...
        with open(info_path,'rb') as f:  # 加载训练集,gt_sample还是从训练集中获取
            infos = pickle.load(f)
        ...
        for k in range(len(infos)):
            ...
            for i in range(num_obj):
                ...
                gt_points[:, :3] -= gt_boxes[i, :3]  # 得到相对于中心点的相对坐标
    ...
    def generate_prediction_dicts(batch_dict,pred_dicts,class_names,output_path = None):
        ...
        def generate_single_sample_dict(box_dict):
            ...
            pred_dict = get_template_prediction(pred_scores.shape[0])  # pred_scores.shape[0] means the number of predicted objects
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        from pcdet.datasets.once_xx_rs.once_eval.evaluation import get_evaluation_results

        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info) for info in self.robosense_infos]
...
def create_robosense_infos(dataset_cfg,class_names,data_path,save_path,workers=4):
    ...
    files_list_label_train = random.sample(files_list_label,int(TRAIN_RATIO_OF_ALL_LABELS*len(files_list_label))) # 按比例随机挑选得到训练集
    files_list_label_val = dataset.list_subtraction(files_list_label,files_list_label_train)  # 得到验证集
    ...
    robosense_infos_train = dataset.get_all_labels(files_list_label=files_list_label_train)  # 返回所有标签数据
...
if __name__ == '__main__':
    ...
    sys.argv.append('create_robosense_infos')
    #sys.argv.append('/media/ztt/ssd1/OpenPCDetBest-SUTENG/tools/cfgs/dataset_configs/rs_dataset4.yaml')
    path1 = '/media/taole/ssd1/letaotao/pcdetlatest/tools/cfgs/dataset_configs/rs_dataset4.yaml'
    dataset_cfg = EasyDict(yaml.safe_load(open(path1)))
    ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
    class_names = ['cyclist',  'vehicle', 'vehicle2','pedestrian']
    
    create_robosense_infos(
        dataset_cfg=dataset_cfg,
        class_names= class_names,
        data_path='/media/taole/ssd1/letaotao/pcdetlatest/data/robosense_datasets/datasets/',
        save_path='/media/taole/ssd1/letaotao/pcdetlatest/data/robosense_datasets/datasets/'
