...
def visualize_pts(pts, fig=None, bgcolor=(0, 0, 0), fgcolor=(1.0, 1.0, 1.0),
                  show_intensity=False, size=(1024,1024 ), draw_origin=False):
    maskx1 = pts[:, 0] < 120
    pts[:,0]=pts[:,0]*maskx1
    maskx2 = pts[:, 0] > -120
    pts[:,0]=pts[:,0]*maskx2
    ...
    if show_intensity:
        ...
        G = mlab.points3d(pts[:, 0], pts[:, 1], pts[:, 2], mode='point',
                          colormap='gnuplot', scale_factor=1, figure=fig)
    if draw_origin:
        mlab.points3d(0, 0, 0, color=(1,0,1), mode='cube', scale_factor=1)
        # mlab.plot3d([0, 3], [0, 0], [0, 0], color=(0, 0, 1), tube_radius=1)
        # mlab.plot3d([0, 0], [0, 3], [0, 0], color=(0, 1, 0), tube_radius=1)
        # mlab.plot3d([0, 0], [0, 0], [0, 3], color=(1, 0, 0), tube_radius=1)
...
def draw_sphere_pts(pts, color=(0, 1, 0), fig=None, bgcolor=(0, 0, 0), scale_factor=0.2):
    ...
    mlab.points3d(0, 0, 0, color=(1, 1, 1), mode='cube', scale_factor=1)
...
def draw_grid(x1, y1, x2, y2, fig, tube_radius=None, color=(0, 0, 0)):
    mlab.plot3d([x1, x1], [y1, y2], [10, 10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
    mlab.plot3d([x2, x2], [y1, y2], [10, 10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
    mlab.plot3d([x1, x2], [y1, y1], [10, 10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
    mlab.plot3d([x1, x2], [y2, y2], [10, 10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
    mlab.plot3d([x1, x1], [y1, y2], [-10, -10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
    mlab.plot3d([x2, x2], [y1, y2], [-10, -10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
    mlab.plot3d([x1, x2], [y1, y1], [-10, -10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
    mlab.plot3d([x1, x2], [y2, y2], [-10, -10], color=color, tube_radius=tube_radius, line_width=1, figure=fig)
...
def draw_multi_grid_range(fig, grid_size=240, bv_range=(-120, -120,120, 120)):
...
def draw_scenes(points, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None):
    ...
    #mlab.view(azimuth=-179, elevation=54.0, distance=100, roll=0)
    return fig
